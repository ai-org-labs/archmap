...
export function traceabilityProjection(model: ArchMapModel, options: LifecycleViewOptions = {}): Projection {
  ...
  const elements = unique(result.elements);
  return {
    title: "Traceability",
    elements,
    relations: relationsFor(model, new Set(elements.map((element) => element.id))),
...
function position(projection: Projection): PositionedElement[] {
  ...
  return grouped.flatMap((elements, column) => elements.map((element, row) => ({
    ...element,
    x: PAD_X + column * (CARD_WIDTH + GAP_X),
    y: PAD_TOP + row * (CARD_HEIGHT + GAP_Y),
    width: CARD_WIDTH,
    height: CARD_HEIGHT,
  })));
}
...
function connector(from: PositionedElement, to: PositionedElement, id: string, type: string): string {
  ...
  return `<g class="archmap-lifecycle-relation" data-id="${escapeXml(id)}"><path d="${path}" fill="none" stroke="#708497" stroke-width="1.5" marker-end="url(#lifecycle-arrow)"/><text x="${labelX}" y="${labelY}" font-size="11" fill="#53687a">${escapeXml(type)}</text></g>`;
}
...
export function renderLifecycleProjection(projection: Projection, className: string): string {
  ...
  const maxRows = Math.max(1, ...projection.columns.map((types) => positioned.filter((element) => types.includes(element.type)).length));
  const width = PAD_X * 2 + projection.columns.length * CARD_WIDTH + Math.max(0, projection.columns.length - 1) * GAP_X;
  const height = PAD_TOP + maxRows * CARD_HEIGHT + Math.max(0, maxRows - 1) * GAP_Y + 34;
  ...
  const cards = positioned.map((element) => {
    ...
    return `<g class="archmap-lifecycle-element archmap-lifecycle-${escapeXml(element.type)}" data-id="${escapeXml(element.id)}"><rect x="${element.x}" y="${element.y}" width="${element.width}" height="${element.height}" rx="6" fill="${color.fill}" stroke="${color.stroke}" stroke-width="1.5"/><text x="${element.x + 14}" y="${element.y + 25}" font-size="11" font-weight="700" fill="${color.stroke}">${escapeXml(element.type)}</text><text x="${element.x + 14}" y="${element.y + 50}" font-size="15" font-weight="600" fill="#16232f">${escapeXml(element.value.title ?? element.value.label ?? element.id)}</text><text x="${element.x + 14}" y="${element.y + 69}" font-size="11" fill="#617283">${escapeXml(element.id)}</text>${rows.map((row, index) => `<text x="${element.x + 14}" y="${element.y + 88 + index * 14}" font-size="10" fill="#526577">${escapeXml(row)}</text>`).join("")}</g>`;
  }).join("");
