...
`),
  },
  {
    ...
    category: "Cloud topology / AWS",
    description: "Redundant web and database path across two Availability Zones, adapted from the Edraw AWS architecture tutorial as direct ArchMap Next communication.",
    recommendation: {
      baseView: "topology",
      renderMode: "2d",
      overlays: ["zone"],
...
topology:
  resources:
    ...
    LoadBalancerC:
      ...
      extensions: { provider: aws, service: elastic-load-balancing }
    WebA:
      label: EC2 Web Primary
      kind: vm
      parent: private_subnet_a
      extensions: { provider: aws, service: ec2 }
    WebC:
      label: EC2 Web Standby
      kind: vm
      parent: private_subnet_c
      extensions: { provider: aws, service: ec2 }
    DatabaseA:
      label: RDS Primary
      kind: relational_database
      parent: private_subnet_a
      extensions: { provider: aws, service: rds }
    DatabaseC:
      label: RDS Standby
      kind: relational_database
      parent: private_subnet_c
  ...
  containers:
    ...
    tokyo_region:
      ...
      kind: aws.region
      roles: [administrative, availability]
      parent: aws_cloud
    production_vpc:
      label: Production VPC
      kind: aws.vpc
...
`),
  ...
  {
    id: "strangler-microservices-4d",
    title: "Strangler-fig microservices (4D)",
    category: "Evolution / 4D",
    description: "Six-phase monolith-to-microservices migration: facade first, per-domain extraction with planned states, a transient dual-write sync worker, deprecation, cutover, and retirement.",
    recommendation: {
      baseView: "overview",
      renderMode: "2d",
      overlays: ["zone", "timeline"],
