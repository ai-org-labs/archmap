...
import type { LayoutEdge, LayoutZone } from "../layout.js";
import {
  buildRuntimeGraph,
  projectRuntimeGraph,
  runtimeGraphToCsv,
...
import type { ArchMapModel, ArchNode } from "../types.js";

interface PositionedNode extends RuntimeGraphNode { x: number; y: number; w: number; h: number; }
interface RuntimeLayout { nodes: PositionedNode[]; edges: LayoutEdge[]; zones: LayoutZone[]; width: number; height: number; }

function layoutGraph(graph: RuntimeGraph, model: ArchMapModel): RuntimeLayout {
  const authored = new Map(model.nodes.map((node) => [node.id, node]));
  const nodes: ArchNode[] = graph.nodes.map((node) => authored.get(node.nodeId) ?? {
    id: node.id,
    label: node.label,
    shape: "rectangle",
    zone: node.zone,
    resolvedZone: node.zone,
    kind: node.kind,
  });
  const runtimeModel: ArchMapModel = {
    ...model,
    nodes,
  ...
  const layout = computeTopologyLayout(runtimeModel);
  const runtimeById = new Map(graph.nodes.map((node) => [node.id, node]));
  return {
    nodes: layout.nodes.flatMap((node) => {
      const runtime = runtimeById.get(node.id);
      return runtime ? [{ ...runtime, x: node.x, y: node.y, w: node.w, h: node.h }] : [];
    }),
    edges: layout.edges,
    zones: layout.zones,
    width: layout.width,
...
function graphSvg(graph: RuntimeGraph, measure: RuntimeMeasure, model: ArchMapModel, selected: string | null = null): string {
  const layout = layoutGraph(graph, model);
  const related = new Set<string>();
  if (selected) {
    related.add(selected);
    graph.edges.forEach((edge) => { if (edge.from === selected || edge.to === selected) { related.add(edge.from); related.add(edge.to); } });
  }
  const runtimeEdges = new Map(graph.edges.map((edge) => [edge.id, edge]));
  const edges = layout.edges.map((route) => {
    const edge = runtimeEdges.get(route.id);
    if (!edge || route.points.length < 2) return "";
    ...
    return `<g class="runtime-edge${faded ? " is-faded" : ""}" data-edge-id="${escapeHtml(edge.id)}"><path d="${path}" style="--edge-width:${edgeWidth(edge, measure)}" marker-end="url(#runtime-arrow)"/><title>${escapeHtml(edge.label ?? edge.protocol ?? edge.id)}</title></g>`;
  }).join("");
  const zones = layout.zones.map((zone) => `<g class="runtime-zone"><rect x="${zone.x}" y="${zone.y}" width="${zone.w}" height="${zone.h}" rx="10"/><text x="${zone.x + 14}" y="${zone.y + 24}">${escapeHtml(zone.label ?? zone.id)}</text></g>`).join("");
  ...
  return `<svg class="archmap-runtime-graph" viewBox="0 0 ${layout.width} ${layout.height}" width="${layout.width}" height="${layout.height}" xmlns="http://www.w3.org/2000/svg"><defs><style>
    .runtime-zone rect{fill:#e8f1f8;fill-opacity:.58;stroke:#a8bed0;stroke-width:1.25}.runtime-zone text{font:700 12px system-ui,sans-serif;fill:#496579}.runtime-edge path{fill:none;stroke:#728399;stroke-width:var(--edge-width);vector-effect:non-scaling-stroke}.runtime-edge.is-faded{opacity:.12}
  ...
  </style><marker id="runtime-arrow" markerUnits="userSpaceOnUse" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8Z"/></marker></defs><g class="runtime-world">${zones}${edges}${nodes}</g></svg>`;
}
  ...
  return { mount(target: Element): ViewHandle {
    ...
    const render = (): void => {
      ...
      root.innerHTML = `${style()}<div class="runtime-query">
        ...
        <div class="runtime-stage">${graphSvg(graph, measure, ctx.model, selected)}${graph.nodes.length ? "" : `<div class="runtime-empty">No runtime snapshot is declared in this ArchMap document.</div>`}</div>
        ...
        <div class="runtime-minimap">${graphSvg(graph, measure, ctx.model)}</div>`;
    ...
    const onClick = (event: Event): void => {
      ...
      if (exportButton) {
        ...
        if (kind === "svg") download(graphSvg(graph, measure, ctx.model), "image/svg+xml", "archmap-runtime.svg");
      }
    };
    ...
    return {
      ...
      exportPng(): Promise<Blob> { return svgToPng(graphSvg(currentGraph(), measure, ctx.model, selected)); },
    };
  }};
}
