...
```js
render(model, { baseView: "overview", overlays: ["boundary", "validation"] });
render(model, { baseView: "zone", overlays: ["auth", "dataflow", "boundary", "permission", "validation"] });
render(model, { baseView: "3d", overlays: ["boundary", "dataflow", "permission", "validation"] });
```
